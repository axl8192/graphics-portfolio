...
public:
    ...
    void setParams(float ifov = 65.0f, float iNearPlane = 0.1f, float iFarPlane = 1000.0f, glm::vec3 iPosition=glm::vec3(4.0f,2.5f,7.0f), float iAngH=-117*pi/180, float iAngV=-18*pi/180, float iSensibility=0.004f, float iVelocity=4.0f){
        fov = ifov;
    ...
    void update(){
        ...
        projMat = glm::perspective(glm::radians(fov), (float)Renderer::getWinWitdh()/(float)Renderer::getWinHeight(), nearPlane, farPlane);
    }
