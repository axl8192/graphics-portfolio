...
private:
    ...
    static std::unordered_map<std::string, float> timers;
...
public:
    //## Base Functions ##//
    static void init(GLFWwindow* window, std::string baseCaption){
        Clock::window  = window;
    ...
    //## Timer Functions ##//
    static void setTimer(std::string name){
        timers[name] = 0.0f;
    }
...
std::unordered_map<std::string, float> Clock::timers;

std::string Clock::baseCaption = "Window";
